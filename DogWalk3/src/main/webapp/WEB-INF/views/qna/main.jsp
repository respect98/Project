		...
		<div class="side_overlay">
					...
					<div class="table-responsive">
							<table class="table">
								...
								<tbody>
									<tr>    				
											<td>${qna.qaindex}</td>											
											<td style="cursor:pointer;color:#blue;" onclick="location.href='detail?qaindex=${qna.qaindex}'">${qna.title}</td>
											<td>${qna.userid}</td>
											<td>${qna.qatime}</td>
											<td>${qna.count}</td>
											<td>${qna.awstate}</td>
			...
			<!-- <nav aria-label="Page navigation example" style="display: none;" id="pagingNav"> -->
					<ul class="pagination" id="pagingview">
						<c:if test="${cpage > 1}">
							<li class="page-item">
								<a class="page-link" href="main?cp=${cpage-1}&ps=${pageSize}" cp="${cpage-1}" ps="${pageSize}" aria-label="Previous">
							...
							<c:choose>
								<c:when test="${cpage==i }">
									<li class="page-item active"><a class="page-link"
										href="main?cp=${i}&ps=${pageSize}" cp="${i}"
										ps="${pageSize}">${i}</a></li>
								</c:when>
								<c:otherwise>
						...
						<c:if test="${cpage < pageCount}">
							...
							<li class="page-item">
								<a class="page-link" href="main?cp=${cpage+1}&ps=${pageSize}"
